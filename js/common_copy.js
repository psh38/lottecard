...
document.querySelectorAll('.slide-wrapper.main-bnr .indicator .bullet').forEach((el, idx) => {
  el.addEventListener('click',()=>{
    clearInterval(mainTimer); // 자동재생 정지
    mainMoveSlide(idx+2); // solid 이동
    mainAutoSlide();  // 자동재생 시작
...
document.querySelectorAll('.control-area .btn-cont').forEach((el, idx) => {
  el.addEventListener('click',(e)=>{
    console.log('재생,정지 기능:', e.target, e.target.classList.contains('stop'))
    if(e.target.classList.contains('stop')){ // 정지 시
...
mainSlideContainer.addEventListener('mouseenter',()=>{
  clearInterval(mainTimer);
});
mainSlideContainer.addEventListener('mouseleave',()=>{
  mainAutoSlide();
});

// 로드시 자동 스와이프 실행(메인 스와이프)
setTimeout(() => {
  mainAutoSlide();
}, 900)
